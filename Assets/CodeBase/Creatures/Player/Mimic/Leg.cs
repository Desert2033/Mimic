...
namespace MimicSpace
{
    public class Leg : MonoBehaviour
    {
        ...
        public void Initialize(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, Mimic myMimic, float lifeTime)
        {
            ...
            RaycastHit hit;
            Physics.Raycast(footPosition + Vector3.up * 5f + new Vector3(footOffset.x, 0, footOffset.y), -Vector3.up, out hit, 200f, _layerMask);
            handles[7] = hit.point;
        ...
        private void Update()
        {
            ...
            else if (growTarget == 1)
            {
                ...
                RaycastHit hit;
                if (Physics.Linecast(footPosition, transform.position, out hit, _layerMask))
                {
                    growTarget = 0;
                }
            }
