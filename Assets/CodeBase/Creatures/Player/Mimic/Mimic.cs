    ...
    public class Mimic : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (legCount <= maxLegs - partsPerLeg)
            {
                ...
                RaycastHit hit;
                Physics.Raycast(newLegPosition + Vector3.up * 10f, -Vector3.up, out hit, 200f, _layerMask);
                Vector3 myHit = hit.point;
                if (Physics.Linecast(transform.position, hit.point, out hit, _layerMask))
                    myHit = hit.point;
        ...
        void RequestLeg(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, Mimic myMimic, float lifeTime)
        {
            ...
            {
                newLeg = InstantiatNewLeg();
            }
            newLeg.SetActive(true);

            InitNewLeg(footPosition, legResolution, maxLegDistance, growCoef, myMimic, lifeTime, newLeg);
        }

        public void InitNewLeg(Vector3 footPosition, int legResolution, float maxLegDistance, float growCoef, Mimic myMimic, float lifeTime, GameObject newLeg)
        {
        ...
        public GameObject InstantiatNewLeg() => 
            Instantiate(legPrefab, transform.position, Quaternion.identity);
